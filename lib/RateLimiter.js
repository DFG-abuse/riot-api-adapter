...
function checkLimit(safeSpace, now, storage){
    return function(limit){
        ...
        var userSet = storage.filter(function(timestamp) {
            return timestamp > clearBefore;
        });

        var tooManyInInterval = userSet.length >= limit.maxInInterval;
        var timeSinceLastRequest = limit.minDifference && (now - userSet[userSet.length - 1]);
        var result;
...
RateLimiter.prototype.check = function(id) {
    ...
    var clearBefore = now - this.maxWindow - this.SAFE_SPACE*1000;
    // aborting deletion
